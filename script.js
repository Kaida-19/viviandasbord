...
async function loadRooms() {
  try {
    ...
    const res = await fetch("https://support-chat-afck.onrender.com/rooms");
...
async function loadHistory(room) {
  try {
    const res = await fetch(
      `https://support-chat-afck.onrender.com/messages/${room}`,
    );
    ...
    messages.forEach((msg) => {
      ...
      div.className =
        "message " + (msg.sender === "admin" ? "admin" : "customer");
      div.innerHTML = `
        ${msg.content}
        <div class="timestamp">
...
async function joinRoom(r) {
  ...
  // 👇 then open websocket for live messages
  ws = new WebSocket(`wss://support-chat-afck.onrender.com/ws/${room}`);
